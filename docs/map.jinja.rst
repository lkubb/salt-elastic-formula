...
We will see a quick setup to configure the ``elastic`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``elastic`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``elastic`` formulas under ``/srv/salt/elastic/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/elastic/parameters/dns:domain/
    ln -s dns:domain /srv/salt/elastic/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/elastic/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/elastic/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``elastic`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``elastic`` formulas under ``/srv/salt/elastic/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/elastic/parameters/roles
...
We create a configuration for the role ``elastic/server`` in ``/srv/salt/elastic/parameters/roles/elastic/server.yaml``:
...
We create another configuration for the role ``elastic/client`` in ``/srv/salt/elastic/parameters/roles/elastic/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles elastic/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles elastic/client
...
.. code-block:: console
    ...
    mv /srv/salt/elastic/parameters/dns:domain/  '/srv/salt/elastic/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-elastic-as-json:
      test.nop:
        - name: {{ elastic | json }}
...
Here is an example based on `template-formula/elastic/config/file.sls`_:
    ...
    elastic-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='elastic-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ elastic.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            elastic: {{ elastic | json }}

This ``sls`` file expose a ``elastic`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ elastic['winner'] }}
...
.. _template-formula/elastic/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/elastic/config/file.sls
