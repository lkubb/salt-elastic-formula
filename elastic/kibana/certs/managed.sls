...
Kibana server certificate private key is managed:
  x509.private_key_managed:
    - name: {{ elastic.lookup.config.kibana | path_join(elastic | traverse("kibana:config:server.ssl:key")) }}
    - algo: rsa
    - keysize: 2048
    - new: true
    - mode: '0660'
...
Kibana server certificate is managed:
  x509.certificate_managed:
    - name: {{ elastic.lookup.config.kibana | path_join(elastic | traverse("kibana:config:server.ssl:certificate")) }}
...
Ensure CA certificates are trusted for Kibana:
  x509.pem_managed:
    - name: {{ elastic.lookup.config.kibana | path_join(elastic | traverse("kibana:config:server.ssl:certificateAuthorities")) }}
